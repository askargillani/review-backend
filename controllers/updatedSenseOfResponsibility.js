...
export const updatedSenseOfResponsibility = async (req,res,next) => {
    try {
        const conversationId = req.query.conversationId;
        const name = req.query.name;
        const conversation = await getContext(conversationId);
        const managerReviews = await getManagerReviews(name);
        var response = await finalReviewService.giveScaleAndCommentOfSenseOfResponsibility(conversation, managerReviews,name);
